...
ここでInference settingsでモデルがデータ解析（＝推論 inference）するときの設定を定めることができます。いくつかのパラメータがありますが特に重要なのは Minimum confidence で、この値を高くすると在判定が厳しくなり、低くすると逆に甘くなります。Minimum confidenceを低くすると他の生き物の鳴き声を検出しやすいのでノイジーになりがちなのですが、珍しい種や外来種で、見落としを避けたいといった場合には低く設定することが有利に働く場面もありえます。ここではデフォルトのまま先に進みましょう。
...
このままの設定でAnalyzeすると、sample_data/testフォルダの中に、「~BirdNET.selection.table.txt」が末尾についたファイルが生成されます。
